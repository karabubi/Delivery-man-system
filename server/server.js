...
// // Optional: DB health check (helps debugging Render)
...
//     if (!deliveries.length) {
//       return res.status(200).json({ startCoordinates, deliveries: [] }); // ✅ don't 404
//     }

//     res.status(200).json({ startCoordinates, deliveries });
//   } catch (err) {
...
//           .filter((d) => d.address && !Number.isNaN(d.position_latitude) && !Number.isNaN(d.position_longitude))
//           .filter((d) => !existingAddresses.includes(d.address));

//         if (!newDeliveries.length) {
//           return res.status(409).json({ error: "All addresses in the file already exist (or invalid rows)." });
//         }
